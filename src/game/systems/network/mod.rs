use bevy::prelude::*;
use heartbeat::{
    check_peer_heartbeat, reply_heartbeat, HeartbeatBundle, HeartbeatStack, HeartbeatTimer,
};
use log::error;
...
pub fn init_network_systems(app: &mut App) {
    app.add_systems(Startup, setup_network)
        ...
        .add_systems(Update, (check_peer_heartbeat, reply_heartbeat));
}

fn setup_network(mut commands: Commands) {
    commands.spawn(HeartbeatBundle {
        timer: HeartbeatTimer(Timer::from_seconds(1.0, TimerMode::Repeating)),
